...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.entryapplicationmain.id == null ? "../entryapplicationmain/save" : "../entryapplicationmain/update";
			$.ajax({
				type: "POST",
			    url: url,
			    data: JSON.stringify(vm.entryapplicationmain),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: "../entryapplicationmain/delete",
        ...
        getRefTreesys_deptdeptid: function(menuId){
            ...
            $.get("../sys_dept/select", function(r){
                ...
                var node = ztreedeptid.getNodeByParam("id", vm.entryapplicationmain.deptid);
                ztreedeptid.selectNode(node);
                vm.entryapplicationmain.deptidname = node.name;
        ...
        openRefTreesys_deptdeptid: function(){
            layer.open({
                ...
                btn1: function (index) {
                    ...
                    //选择上级菜单
                    vm.entryapplicationmain.deptid = node[0].id;
                    vm.entryapplicationmain.deptidname = node[0].name;
            ...
            submitworkflow:function () {
                ...
                confirm('确定要提交选中的任务？', function () {
                    $.ajax({
                        type: "POST",
                        url: "../entryapplicationmain/submitworkflow",
            ...
            copybill: function (event) {
                ...
                $.get("../entryapplicationmain/info/" + id, function (r) {
                    vm.entryapplicationmain = r.entryapplicationmain;
                    vm.entryapplicationmain.id=null;
                    vm.entryapplicationmain.billno="*";
                });
            ...
            cancel: function (event) {
                ...
                confirm('确定要作废选中的单据？', function(){
                    $.ajax({
                        type: "POST",
                        url: "../entryapplicationmain/cancel",
                        data: JSON.stringify(ids),
                        success: function(r){
                            if(r.code == 0){
                                alert('操作成功', function(index){
        ...
        claim:function(event){
            ...
            confirm('确定要签收选中的任务？', function(){
                $.ajax({
                    type: "POST",
                    url: "../entryapplicationmain/claim",
                    data: JSON.stringify(ids),
                    success: function(r){
                        if(r.code == 0){
                            alert('操作成功', function(index){
        ...
        audit:function(event){
            ...
            $.get("../entryapplicationmain/info/"+id, function(r){
                ...
                showrefgrid_billcomments(function (data) {
                    if(!vm.entryapplicationmain.billcommentsEntity){
                        vm.entryapplicationmain.billcommentsEntity={};
                    }
                    vm.entryapplicationmain.billcommentsEntity.billno=vm.entryapplicationmain.billno;
                    vm.entryapplicationmain.billcommentsEntity.refbilltype=0;
                    vm.entryapplicationmain.billcommentsEntity.remark=data.remark;
                    vm.entryapplicationmain.billcommentsEntity.auditstatus=data.auditstatus;
                    $.ajax({
                        type: "POST",
                        url: "../entryapplicationmain/audit",
                        data: JSON.stringify(vm.entryapplicationmain),
                        success: function(r){
                            if(r.code == 0){
                                alert('操作成功', function(index){
        ...
        unaudit:function(){
            ...
            confirm('确定要反审核选中的记录？', function(){
                $.ajax({
                    type: "POST",
                    url: "../entryapplicationmain/unaudit",
                    data: JSON.stringify(ids),
                    success: function(r){
                        if(r.code == 0){
                            alert('操作成功', function(index){
        ...
        upload_on_success:function (response,file,fileList) {
            ...
            if(!vm.entryapplicationmain.files){
                vm.entryapplicationmain.files=[];
            }
            if(response.page.list&&response.page.list.length>0){
                vm.entryapplicationmain.files.push(response.page.list[0]);
            }
